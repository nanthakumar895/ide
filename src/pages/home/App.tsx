...
const App = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0b0e14] text-slate-200 font-sans overflow-hidden relative">
      ...
      {/* Side Menu Drawer */}
      <aside
        className={`fixed top-0 left-0 bottom-0 w-[280px] bg-[#1a1d23] z-[70] transform transition-transform duration-300 ease-in-out border-r border-white/5 flex flex-col ${
      ...
      >
        ...
        <div className="flex-1 p-4 space-y-1 overflow-y-auto">
          {[
            ...
            { icon: Zap, label: 'Premium', sub: 'Unlock solutions', highlight: true, link: '/premium.html' },
            ...
            >
              <item.icon size={20} className={item.highlight ? 'text-[#ff5a00]' : 'group-hover:text-white'} />
      ...
      <header className="flex-none bg-[#0b0e14] px-4 h-14 flex items-center justify-between z-50 border-b border-white/5">
        <div className="flex items-center gap-4">
          <button onClick={() => setIsMenuOpen(true)} className="p-1 text-slate-400 hover:text-white transition-colors">
            <Menu size={24} />
          </button>
        ...
        <div className="flex items-center gap-4">
          ...
          <div onClick={() => setIsMenuOpen(true)} className="w-8 h-8 rounded-full bg-purple-500 flex items-center justify-center text-[10px] font-bold text-white border border-white/10 cursor-pointer hover:ring-2 ring-white/20 transition-all">JD</div>
        </div>
      </header>

      {/* Content */}
      <main className="flex-1 overflow-y-auto pb-28 pt-4 custom-scrollbar">
